...
def load_tilemap(file_path, clut, tileset):
    ...
    for line in lines:
        ...
        for byte_str in bytes_in_line:
            ...
            # For now, we just append the first byte (constructed from CLUT and TileSet)
            # and the original byte_value as the second byte
            converted_tilemap.append((first_byte, byte_value))

    return converted_tilemap

def save_converted_tilemap(output_path, converted_tilemap):
    with open(output_path, 'w') as file:
        for first_byte, second_byte in converted_tilemap:
            # Write the converted bytes back in a comma-delimited format
            file.write(f"{first_byte},{second_byte}\n")
...
if __name__ == "__main__":
    ...
    # Output file for the converted map
    output_file = "converted_tilemap.txt"

    # Save the converted map
    save_converted_tilemap(output_file, converted_tilemap)

    print(f"Converted tile map saved to {output_file}")
